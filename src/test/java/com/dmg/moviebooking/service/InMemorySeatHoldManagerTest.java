...
class InMemorySeatHoldManagerTest {
    ...
    void holdSeats_ShouldSucceed_WhenSeatsAreAvailable() {
        boolean result = holdManager.holdSeats(1L, Set.of(1L, 2L, 3L), "user1", 5);
        assertTrue(result);
    }

    @Test
    void holdSeats_ShouldFail_WhenSeatsAlreadyHeldByAnotherUser() {
        holdManager.holdSeats(1L, Set.of(1L, 2L), "user1", 5);

        boolean result = holdManager.holdSeats(1L, Set.of(2L, 3L), "user2", 5);
        assertFalse(result);
    }
    ...
    void releaseSeats_ShouldRemoveHolds() {
        holdManager.holdSeats(1L, Set.of(1L, 2L, 3L), "user1", 5);

        holdManager.releaseSeats(1L, Set.of(1L, 2L));

        assertFalse(holdManager.isSeatHeld(1L, 1L, "user1"));
        assertFalse(holdManager.isSeatHeld(1L, 2L, "user1"));
        assertTrue(holdManager.isSeatHeld(1L, 3L, "other"));
    }

    @Test
    void getAllHeldSeatIds_ShouldReturnOnlyActiveHolds() {
        holdManager.holdSeats(1L, Set.of(1L, 2L, 3L), "user1", 5);
        holdManager.holdSeats(2L, Set.of(4L, 5L), "user2", 5);
    ...
    void getHeldSeatsByUser_ShouldReturnOnlyUserHolds() {
        holdManager.holdSeats(1L, Set.of(1L, 2L), "user1", 5);
        holdManager.holdSeats(1L, Set.of(3L, 4L), "user2", 5);

        Set<Long> user1Held = holdManager.getHeldSeatsByUser(1L, "user1");
        Set<Long> user2Held = holdManager.getHeldSeatsByUser(1L, "user2");
    ...
    void releaseSeatsByUser_ShouldRemoveOnlyThatUsersHolds() {
        ...
        holdManager.releaseSeatsByUser(1L, "user1");

        assertFalse(holdManager.isSeatHeld(1L, 1L, "any"));
        assertFalse(holdManager.isSeatHeld(1L, 2L, "any"));
        assertTrue(holdManager.isSeatHeld(1L, 3L, "other"));
        assertTrue(holdManager.isSeatHeld(1L, 4L, "other"));
    }

    @Test
    void getHoldingUser_ShouldReturnCorrectUser() {
        holdManager.holdSeats(1L, Set.of(1L), "user1", 5);

        assertEquals("user1", holdManager.getHoldingUser(1L, 1L));
    ...
    void extendHold_ShouldExtendDuration() {
        ...
        boolean result = holdManager.extendHold(1L, Set.of(1L, 2L), "user1", 5);
    ...
    void extendHold_ShouldFail_WhenSeatHeldByDifferentUser() {
        ...
        boolean result = holdManager.extendHold(1L, Set.of(1L), "user2", 5);
    ...
    void cleanupExpiredHolds_ShouldRemoveExpiredEntries() {
        holdManager.holdSeats(1L, Set.of(1L, 2L), "user1", -1); // Already expired
